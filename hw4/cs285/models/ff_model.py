...
class FFModel(BaseModel):
    ...
    def define_forward_pass(self):
        ...
        obs_normalized = normalize(obs_unnormalized, self.obs_mean_pl, self.obs_std_pl) # DONE(Q1) Define obs_normalized using obs_unnormalized,and self.obs_mean_pl and self.obs_std_pl
        acs_normalized = normalize(acs_unnormalized, self.acs_mean_pl, self.acs_std_pl) # DONE(Q2) Define acs_normalized using acs_unnormalized and self.acs_mean_pl and self.acs_std_pl
    ...
    def define_train_op(self):
        ...
        self.delta_labels_normalized = normalize(self.delta_labels, self.delta_mean_pl, self.delta_std_pl) # DONE(Q1) Define a normalized version of delta_labels using self.delta_labels (which are unnormalized), and self.delta_mean_pl and self.delta_std_pl

        # compared predicted deltas to labels (both should be normalized)
        self.loss = tf.losses.mean_squared_error(labels=self.delta_labels_normalized,
                                                predictions=self.delta_pred_normalized) # DONE(Q1) Define a loss function that takes as input normalized versions of predicted change in state and ground truth change in state
        self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss) # DONE(Q1) Define a train_op to minimize the loss defined above. Adam optimizer will work well.
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        return self.sess.run(self.next_obs_pred, feed_dict=feed_dict) # DONE(Q1) Run model prediction on the given batch of data

    def update(self, observations, actions, next_observations, data_statistics):
        # train the model
        feed_dict = {self.delta_labels: next_observations - observations,
