...
class Histogram(Density_Model):
    ...
    def _apply_to_states(self, states, function):
    ...
    def _get_count(self, bin_name):
        return self.hist[bin_name]

    def _get_prob(self, bin_name):
        return self._get_count(bin_name)/float(self.total)

    def get_count(self, states):
        return self._apply_to_states(states, self._get_count)

    def get_prob(self, states):
        return self._apply_to_states(states, self._get_prob)
    ...
    #     """
    #     # raise NotImplementedError
    #     probs = [None]*len(states)
    #     for i, state in enumerate(states):
    #         bin_name = self.preprocessor(state)
    #         probs[i] = self.hist[bin_name]/float(self.total)
...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            densities = gaussians.mean(axis=1)
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        # raise NotImplementedError
        self.elbo = tf.math.reduce_mean(self.log_likelihood - self.kl) # None
        self.update_op = tf.train.AdamOptimizer(self.learning_rate).minimize(-self.elbo) # None
